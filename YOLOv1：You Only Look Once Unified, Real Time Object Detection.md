...
bounding box的预测包括**x, y, w, h**四个值。**x和y表示bounding box的中心相对于图片左上角的cell坐标**。（自注：这里的“坐标”是以cell单元格在宽高方向上的个数为基准，比如x=1.3，y=4.7说明bbox中心位置在x方向上走1.3个cell，然后y方向走4.7个cell，每个cell的边长=448/7=64，而不是以图片实际实际宽高为基准）；w和h则是相对于整张图片的宽高进行归一化的。
